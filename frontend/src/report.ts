...
export function downloadExecutiveReport(data: Analysis, auditEvents: AuditEvent[]) {
  ...
  const approved = auditEvents.some(
    (event) => event.action === "analysis.approved" && event.analysis_id === data.governance.analysis_id,
  );
  const reportStatus = approved ? "Approved for the declared purpose" : "DRAFT — not reviewed and approved";
  ...
  const auditRows = auditEvents
    .map(
      (event) =>
        `<tr><td>${escapeHtml(event.action)}</td><td>${escapeHtml(new Date(event.timestamp * 1000).toISOString())}</td><td>${escapeHtml(event.actor_email || event.actor_id)} (${escapeHtml(event.actor_role)})</td><td><code>${escapeHtml(event.signature)}</code></td></tr>`,
    )
    .join("");
  const html = `<!doctype html>
<html lang="en"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width">
<title>DataLens ${approved ? "approved brief" : "draft profile"} · ${escapeHtml(data.filename)}</title>
<style>
...
<header class="top"><div><div class="brand">DATALENS</div><h1>${approved ? "Approved data brief" : "Draft structural profile"}</h1><p>${escapeHtml(data.filename)}</p><strong>${escapeHtml(reportStatus)}</strong></div><div class="meta">Generated ${escapeHtml(generatedAt)}<br>Session-only analysis</div></header>
...
<section><h2>Governance declaration</h2><div class="method">Analysis ID: ${escapeHtml(data.governance.analysis_id)}<br>Owner: ${escapeHtml(data.governance.declared.owner || "Not declared")}<br>Classification: ${escapeHtml(data.governance.declared.classification || "Not declared")}<br>Source: ${escapeHtml(data.governance.declared.source_url || "Not declared")}<br>Source checked by owner: ${escapeHtml(data.governance.declared.verified_at || "Not declared")}<br>Purpose: ${escapeHtml(data.governance.declared.purpose || "Not declared")}<br>Authorization attested: ${data.governance.declared.authorized_to_process === true ? "Yes" : "No"}<br>Application retention: no intentional persistence; transient framework and hosting processing may occur.</div></section>
...
<section><h2>Session review record</h2>${auditRows ? `<table><thead><tr><th>Event</th><th>UTC timestamp</th><th>Actor</th><th>Full service signature</th></tr></thead><tbody>${auditRows}</tbody></table>` : "<p>No review event was recorded.</p>"}<p>The service signature protects the event metadata while the signing key remains controlled. This downloaded HTML is not centrally retained and can be edited after download, so it is not a durable or independently verifiable audit ledger.</p></section>
