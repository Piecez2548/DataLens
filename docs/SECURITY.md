...
Version 0.6.0 requires authentication in production, validates roles server-side, keeps credentials in deployment environment variables, signs metadata-only audit events, and does not persist uploaded datasets. The release pipeline runs lint, unit/integration tests, browser E2E, npm vulnerability audit, and Python dependency audit.
